...
def create_osr_train_list():
    text_file=os.path.join('/root/yinxu/original/object-centric-learning-framework/exps/coco2014/coco2014_train.txt')
    #print(text_file)
    line_list=open(text_file).readlines()
    #print("Totally have {} samples in train set.".format(len(line_list)))
    img_path=[]
    aug_str=''
    for idx,line in enumerate(line_list):
        name=line.split()[0].split('/')[1]
        img_path.append(name[:4]+aug_str+name[4:])
        #print(img_path)
    return img_path
def create_osr_cls_train_list():
    mat_file=os.path.join('/root/yinxu/original/object-centric-learning-framework/exps/voc/voc12-train.mat')
    #print(mat_file)
    line_list=loadmat(mat_file)['Imlist']
    print("Totally have {} samples in train set.".format(len(line_list)))
    img_path=[]
    aug_str=''
    for line in line_list:
        #print(list(str(line[idx)))
        name=str(line)[-26:]
        img_path.append(name)
        #print(name)
    return img_path

def create_voc_detection_train_list():
    text_file=os.path.join('/root/yinxu/original/object-centric-learning-framework/exps/voc_detection/train.txt')
    #print(mat_file)
    line_list=open(text_file).readlines()
    print("Totally have {} samples in train set.".format(len(line_list)))
    img_path=[]
...
def main(
    ...
    osr_name_list=create_voc_detection_train_list()
    #print(osr_name_list)
    #image_ids = list(osr_name_list)  # Make copy.
    print("Number of instances:", len(osr_name_list))
    random.seed(seed)
    random.shuffle(osr_name_list)  # Ensure instances are shuffled.
    
    instance_count = 0
    with webdataset.ShardWriter(get_shard_pattern(output_path), **shard_writer_params) as writer:
        for image_id in tqdm.tqdm(osr_name_list):
            output = {}
            image_id=image_id.strip()[2:]
            output["__key__"] = str(image_id.split('/')[-1][:-4])
            #print(str(image_id.split('/')[-1][:-4]))
