...
def create_osr_train_list():
    text_file=os.path.join('/root/yinxu/original/object-centric-learning-framework/exps/osr_cifar50/train/train_cifar100_50_seed1.txt')
    #print(text_file)
...
def main(
    ...
    osr_name_list=create_osr_train_list()
    #print(osr_name_list)
    instance_count = 0
    with webdataset.ShardWriter(get_shard_pattern(output_path), **shard_writer_params) as writer:
        for image_id in tqdm.tqdm(osr_name_list):
            ...
            #print(image_id.split('/')[-1][:-5])
            output["__key__"] = str(image_id.split('/')[-1][:-5])
