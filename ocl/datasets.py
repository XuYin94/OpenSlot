...
class CustomdatasetDataModule(pl.LightningDataModule):
    def __init__(
        self,
        train_shards: Optional[Union[str, List[str]]] = None,
        val_shards: Optional[Union[str, List[str]]] = None,
...
class Discovery_dataset(Dataset):
    def __init__(self, data_root="D:\\datasets\\VOC\\VOCdevkit\\",prefix='train',split_idx="1", exp="voc", transform=None):
        ...
        # if exp=="voc":
        #     self.num_known_classes=5
        # elif exp=="coco":
        #     self.num_known_classes=16

    def __get_name_list(self):
        img_path = []
        labels = []
        for txt in self.prefix:
            text_file=os.path.join("./exps/"+self.exp+"/"+self.exp+"_"+str(txt)+"_split_"+str(self.split_idx)+".txt")
            print(text_file)
            line_list=open(text_file).readlines()
            print("Totally have {} samples in {} set.".format(len(line_list),txt))
