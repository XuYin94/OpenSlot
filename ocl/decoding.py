...
from ocl.neural_networks.convenience import build_two_layer_mlp,build_mlp
import torch
...
class OSR_classifier(nn.Module):
    """MLP classifier used after discovery exp."""
    ...
    activation_fn: str,
        ...
        if initial_layer_norm:
            mlp_classifier.append(nn.LayerNorm(input_dim))


        mlp_classifier.extend(
            [nn.Linear(input_dim,input_dim),
            get_activation_fn(activation_fn),
            nn.Linear(input_dim,nbr_clses)]
        )
        self.mlp_classifier=nn.Sequential(*mlp_classifier)
    def forward(self,object_features:torch.Tensor):
        return self.mlp_classifier(object_features)
