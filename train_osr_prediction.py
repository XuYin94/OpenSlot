...
def OSR_fit(config,model,train_loader,val_loader,test_loader_list,nbr_cls,visualizer,device):
    ...
    msg=model.load_state_dict(weight)
    print(msg)
    ...
    #         slots,logits,fg_matching_loss = \
    #             outputs["slots"],outputs["fg_pred"],outputs["fg_matching_loss"]
    #
    #
    #         loss=fg_matching_loss
    #         loss.backward()
    ...
    #     decoder_output = model.get_slot_attention_mask(image,phase="train")
    ...
    #     print("Train | Epoch: {}, Loss: {:5f}, Fg_loss: {:5f},Acc: {:5f}, LR: {}".format(
    #         epoch, total_loss,fg_loss,acc,lr_scheduler.get_last_lr()))
...
def overall_test(evaluator,model, val_loader, test_loader_list, epoch, writer):
    evaluator.eval(model, val_loader, test_loader_list, epoch, writer, processor=["slotenergy","slotmax","slotmin"],oscr=True)
